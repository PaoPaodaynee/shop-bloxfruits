...
// TÀI KHOẢN VÀ MẬT KHẨU ADMIN CỦA BẠN
...
app.post('/api/register', (req, res) => {
    ...
    const newUser = { username, password, balance: 100000, role: "user" };
    users.push(newUser);
    res.json({ success: true, message: "Đăng ký thành công! Đã tặng bạn 100.000đ trải nghiệm.", user: newUser });
});

// 2. API ĐĂNG NHẬP (NHẬN DIỆN KHÁCH HOẶC SẾP)
app.post('/api/login', (req, res) => {
    ...
    if (username === ADMIN_CREDENTIALS.username && password === ADMIN_CREDENTIALS.password) {
        return res.json({
            success: true,
            isAdmin: true,
            message: "Xin chào Sếp OTOPI! Đang bay sang trang đăng acc...",
            adminToken: ADMIN_SECRET_KEY,
            user: { username: "ADMIN", role: "admin", balance: 999999999 }
        });
    }

    // NẾU LÀ KHÁCH HÀNG THƯỜNG
